...
!!! compat "Lazy Operator Memory Optimization"
    All time evolution methods work seamlessly with [lazy operators](@ref doc-Lazy-Operators) for memory-efficient HEOMLS matrices. Simply construct the HEOMLS matrix with `assemble = Val(:combine)` and pass it to [`HEOMsolve`](@ref) as usual. See [Lazy Operator Memory Optimization](@ref doc-Lazy-Operators) for details on reducing memory usage.
